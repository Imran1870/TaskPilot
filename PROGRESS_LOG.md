...
* **Docker Dependency Resolve:** Fixed build-stage dependency resolution by copying `shared` inside `/app/shared` and symlinking `/shared` in both Dockerfiles. Added root welcome `GET /` and favicon `GET /favicon.ico` route handlers to eliminate 404 error logging clutter. Aligned `calendarController.js` OAuth callback redirects with `config.clientUrl` for production routing. Corrected local and Gemini voice category parser enums to map exactly to the database schema enums, resolving Zod save errors. Added `deploy-setup.md` documentation.
