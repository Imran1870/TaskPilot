...
export const Login = () => {
  ...
  // Handle callback query parameters on mount
  useEffect(() => {
    ...
    if (token) {
      const handleTokenLogin = async () => {
        ...
        if (res.success) {
          ...
          setSubmitError(res.error || 'Failed to authenticate with Google');
        }
      };
      handleTokenLogin();
    } else if (error) {
      if (error === 'access_denied') {
        setSubmitError('Google sign-in was cancelled or access was denied.');
      } else {
        setSubmitError(`Google authentication error: ${error.replace(/_/g, ' ')}`);
      }
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (data.authUrl) {
        ...
        setSubmitError('Failed to retrieve Google auth URL');
        setLoading(false);
      }
    } catch (err) {
        ...
        <div className="bg-[#0b101f] border border-slate-800 rounded-[28px] p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-[#0b101f] px-3 text-slate-500 font-semibold">Or continue with</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="flex w-full justify-center items-center gap-3 rounded-full border border-slate-800 bg-[#0d1527] py-3 px-4 text-sm font-bold text-slate-350 hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-700 transition-all disabled:opacity-50"
          >
